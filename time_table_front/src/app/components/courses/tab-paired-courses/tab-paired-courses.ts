...
export class TabPairedCoursesComponent implements OnInit {
  ...
  readonly selectedContextMenuGroup = signal<PairedGroupView | null>(null);

  readonly contextMenuItems = computed<ContextMenuItem[]>(() => {
    const group = this.selectedContextMenuGroup();
    if (!group) return [];

    return [
      {
        id: 'edit',
        label: 'แก้ไขกลุ่มวิชาคู่',
        sublabel: `มีทั้งหมด ${group.courses.length} วิชา`,
        icon: 'edit',
        iconType: 'edit',
        action: () => this.openEditModal(group),
      },
      {
        id: 'detail',
        label: 'ดูรายละเอียดวิชาในกลุ่ม',
        sublabel: group.courses.map((c) => c.COURSE_NO).join(', '),
        icon: 'visibility',
        iconType: 'detail',
        dividerAfter: true,
        action: () => this.openDetailModal(group),
      },
      {
        id: 'delete',
        label: 'ลบกลุ่มวิชาคู่',
        sublabel: `กลุ่มที่ ${group.groupId} (ย้ายลง HIS)`,
        icon: 'delete',
        iconType: 'delete',
        variant: 'danger',
        action: () => this.deleteGroup(group.groupId),
      },
    ];
  });

  onRowContextMenu(event: MouseEvent, group: PairedGroupView): void {
    event.preventDefault();
    event.stopPropagation();
    this.selectedContextMenuGroup.set(group);
    this.contextMenuPos.set({ x: event.clientX, y: event.clientY });
    this.isContextMenuOpen.set(true);
  }

  openMenuFromBtn(event: MouseEvent, group: PairedGroupView): void {
    event.preventDefault();
    event.stopPropagation();
    this.selectedContextMenuGroup.set(group);
  ...
  async deleteGroup(groupId: number): Promise<void> {
    ...
    const confirmed = await this.confirmDialogService.confirm({
      title: 'ยืนยันการลบกลุ่มวิชาคู่',
      message: `คุณต้องการลบ "กลุ่มวิชาคู่ที่ ${groupId}" ใช่หรือไม่?`,
      detail: 'ระบบจะจัดเก็บประวัติการลบไว้ในตาราง HIS ให้อัตโนมัติ',
      confirmText: 'ลบกลุ่มนี้',
      cancelText: 'ยกเลิก',
      variant: 'danger',
    });
  ...
  async deleteSelectedGroups(): Promise<void> {
    if (this.isDeleting()) return;
    ...
    const confirmed = await this.confirmDialogService.confirm({
      ...
      message: `คุณต้องการลบกลุ่มวิชาคู่ที่เลือกจำนวน ${ids.length} กลุ่ม ใช่หรือไม่?`,
      detail: 'ระบบจะจัดเก็บประวัติการลบทั้งหมดไว้ในตาราง HIS',
      confirmText: `ลบ ${ids.length} กลุ่ม`,
      cancelText: 'ยกเลิก',
      variant: 'danger',
    });
    ...
    this.http
      ...
      .subscribe({
        next: (res) => {
          this.isDeleting.set(false);
          this.toastService.success(`ลบ ${ids.length} กลุ่มวิชาคู่สำเร็จ`);
          this.selectedGroupIds.set([]);
          this.loadPairCoursesList();
        },
        error: (err) => {

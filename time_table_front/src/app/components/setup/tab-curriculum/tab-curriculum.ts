...
export class TabCurriculumComponent implements OnInit {
  ...
  readonly contextMenuBadge = signal<string | undefined>(undefined);
  readonly contextMenuTitle = signal<string | undefined>(undefined);
  readonly contextMenuItems = signal<ContextMenuItem[]>([]);

  onGroupContextMenu(event: MouseEvent, grp: CurriculumGroupView): void {
    event.preventDefault();
    event.stopPropagation();
    this.contextMenuBadge.set(grp.groupNo);
    this.contextMenuTitle.set(grp.groupName);
    this.contextMenuItems.set([
      {
        id: 'view',
        label: 'ดูรายละเอียดรายวิชา',
        sublabel: `มีทั้งหมด ${grp.totalCourses} วิชา (${grp.totalCredits} หน่วยกิต)`,
        icon: 'visibility',
        iconType: 'detail',
        dividerAfter: true,
        action: () => this.openGroupDetailModal(grp),
      },
      {
        id: 'delete',
        label: 'ลบกลุ่มหลักสูตร',
        sublabel: `ลบทั้งหมด ${grp.totalCourses} วิชา`,
        icon: 'delete',
        iconType: 'delete',
        variant: 'danger',
        action: () => this.deleteCurriculumGroup(grp),
      },
    ]);
  ...
  openGroupMenuFromBtn(event: MouseEvent, grp: CurriculumGroupView): void {
    event.preventDefault();
    event.stopPropagation();
  ...
  onCourseContextMenu(event: MouseEvent, c: CurriculumCourseRow): void {
    event.preventDefault();
    event.stopPropagation();
    this.contextMenuBadge.set(c.COURSE_NO);
    this.contextMenuTitle.set(c.COURSE_NAME_THAI || c.COURSE_NAME_ENG_L || 'วิชาในหลักสูตร');
    this.contextMenuItems.set([
      {
        id: 'delete-course',
        label: 'ลบวิชานี้ออกจากหลักสูตร',
        sublabel: `วิชา ${c.COURSE_NO}`,
        icon: 'delete',
        iconType: 'delete',
        variant: 'danger',
        action: () => this.deleteCurriculumCourse(c),
      },
    ]);
    this.contextMenuPos.set({ x: event.clientX, y: event.clientY });
    this.isContextMenuOpen.set(true);
  ...
  openCourseMenuFromBtn(event: MouseEvent, c: CurriculumCourseRow): void {
    event.preventDefault();
    event.stopPropagation();
    const rect = (event.currentTarget as HTMLElement).getBoundingClientRect();
    this.contextMenuBadge.set(c.COURSE_NO);
  ...
  // Group Detail Modal State (View Eye Button)
  readonly isGroupDetailModalOpen = signal<boolean>(false);
  readonly selectedGroupDetail = signal<CurriculumGroupView | null>(null);
  readonly modalDetailSearch = signal<string>('');

  // Computed: Grouped Curriculums (1 row per Program Group + Sub Group)
  readonly groupedCurriculums = computed<CurriculumGroupView[]>(() => {
    const q = this.searchQuery().trim().toLowerCase();
    const fac = this.filterFaculty();
    const grp = this.filterGroup();
    const allCourses = this.curriculumCourses();

    const groupMap = new Map<string, CurriculumGroupView>();

    for (const c of allCourses) {
      const subNo = c.SUB_GROUP_NO || '00';
      const key = `${c.FACULTY_NO}_${c.GROUP_NO}_${subNo}`;

      if (!groupMap.has(key)) {
        groupMap.set(key, {
          key,
          facultyNo: c.FACULTY_NO,
          facultyNameThai: c.FACULTY_NAME_THAI,
          facultyNameShort: c.FACULTY_NAME_SHORT,
          groupNo: c.GROUP_NO,
          groupName: c.GROUP_NAME || `กลุ่ม ${c.GROUP_NO}`,
          subGroupNo: subNo,
          subGroupName: (subNo !== '00' && c.SUB_GROUP_NAME) ? `${c.SUB_GROUP_NAME}` : (subNo !== '00' ? subNo : 'กลุ่มหลัก'),
          courses: [],
          totalCourses: 0,
          totalCredits: 0,
        });
      ...
      const grpItem = groupMap.get(key)!;
      grpItem.courses.push(c);
      grpItem.totalCourses++;
      grpItem.totalCredits += (Number(c.CREDIT) || 0);
    }

    let result = Array.from(groupMap.values());

    if (fac !== 'ALL') {
      result = result.filter((g) => g.facultyNo === fac);
    }
    if (grp !== 'ALL') {
      result = result.filter((g) => g.groupNo === grp);
    }

    if (q) {
      result = result.filter((g) => {
        const matchGrp = g.groupName?.toLowerCase().includes(q) || g.groupNo.toLowerCase().includes(q);
        const matchSub = g.subGroupName?.toLowerCase().includes(q) || g.subGroupNo.toLowerCase().includes(q);
        const matchFac = g.facultyNameThai?.toLowerCase().includes(q) || g.facultyNameShort?.toLowerCase().includes(q);
        const matchAnyCourse = g.courses.some((c) =>
          c.COURSE_NO.toLowerCase().includes(q) ||
          c.COURSE_NAME_THAI?.toLowerCase().includes(q) ||
          c.COURSE_NAME_ENG_L?.toLowerCase().includes(q)
        );
        return matchGrp || matchSub || matchFac || matchAnyCourse;
      });
    }
  ...
  readonly totalItems = computed(() => this.groupedCurriculums().length);
  ...
  readonly paginatedCurriculums = computed(() => {
    const list = this.groupedCurriculums();
  ...
  // Filtered courses inside the Group Detail Modal
  readonly modalDetailCourses = computed(() => {
    const detail = this.selectedGroupDetail();
    if (!detail) return [];
    const q = this.modalDetailSearch().trim().toLowerCase();
    if (!q) return detail.courses;

    return detail.courses.filter((c) =>
      c.COURSE_NO.toLowerCase().includes(q) ||
      c.COURSE_NAME_THAI?.toLowerCase().includes(q) ||
      c.COURSE_NAME_ENG_L?.toLowerCase().includes(q) ||
      c.YEAR_LEVEL.includes(q) ||
      c.SEMESTER.includes(q)
    );
  });

  openGroupDetailModal(grp: CurriculumGroupView): void {
    this.modalDetailSearch.set('');
    this.selectedGroupDetail.set(grp);
    this.isGroupDetailModalOpen.set(true);
  }

  closeGroupDetailModal(): void {
    this.isGroupDetailModalOpen.set(false);
    this.selectedGroupDetail.set(null);
    this.modalDetailSearch.set('');
  }

  async deleteCurriculumGroup(group: CurriculumGroupView): Promise<void> {
    const groupTitle = `${group.groupName || ('กลุ่ม ' + group.groupNo)} ${group.subGroupNo !== '00' ? '(' + group.subGroupName + ')' : ''}`.trim();
    const confirmed = await this.confirmDialogService.confirm({
      title: 'ยืนยันการลบกลุ่มหลักสูตร',
      message: `คุณต้องการลบ "${groupTitle}" (มีรายวิชาทั้งหมด ${group.totalCourses} วิชา) ใช่หรือไม่?`,
      detail: 'ระบบจะจัดเก็บประวัติการลบทั้งหมดไว้ในตาราง HIS ให้อัตโนมัติ',
      confirmText: `ลบ ${group.totalCourses} วิชาในกลุ่มนี้`,
      cancelText: 'ยกเลิก',
      variant: 'danger',
    });
    ...
    const itemsToDelete = group.courses.map((r) => ({
    ...
    this.http
      ...
      .subscribe({
        next: () => {
          ...
          this.toastService.success(`ลบกลุ่มหลักสูตรสำเร็จ (${itemsToDelete.length} วิชา)`);
          this.loadCurriculumList();
        },
        error: (err) => {
          this.isDeleting.set(false);
          this.toastService.error(err?.error?.message || 'ลบไม่สำเร็จ');
        },
      });
  }
  ...
  async deleteCurriculumCourse(row: CurriculumCourseRow): Promise<void> {
    ...
    const confirmed = await this.confirmDialogService.confirm({
      ...
      detail: 'ระบบจะจัดเก็บประวัติการลบไว้ในตาราง HIS ให้อัตโนมัติ',
  ...
  async deleteSelectedCourses(): Promise<void> {
    const keys = this.selectedKeys();
    if (keys.length === 0 || this.isDeleting()) return;

    const confirmed = await this.confirmDialogService.confirm({
      title: 'ยืนยันการลบแบบกลุ่ม',
      message: `คุณต้องการลบวิชาในหลักสูตรที่เลือกทั้งหมด ${keys.length} รายการ ใช่หรือไม่?`,
      detail: 'ระบบจะจัดเก็บประวัติการลบทั้งหมดไว้ในตาราง HIS',
      confirmText: `ลบ ${keys.length} รายการ`,
      cancelText: 'ยกเลิก',
      variant: 'danger',
    });
    ...
    this.http
      ...
      .subscribe({
        next: (res) => {
          this.isDeleting.set(false);
          this.toastService.success(`ลบ ${itemsToDelete.length} วิชา สำเร็จ`);
          this.selectedKeys.set([]);
          this.loadCurriculumList();
        },
        error: (err) => {

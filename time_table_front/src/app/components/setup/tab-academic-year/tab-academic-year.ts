...
export class TabAcademicYearComponent implements OnInit {
  ...
  readonly selectedContextMenuRecord = signal<YearSemItem | null>(null);

  readonly contextMenuItems = computed<ContextMenuItem[]>(() => {
    const item = this.selectedContextMenuRecord();
    if (!item) return [];

    const items: ContextMenuItem[] = [];

    // Set Active Option (if not active currently)
    if (item.STUDY_ACTIVE !== '1') {
      items.push({
        id: 'set-active',
        label: 'กำหนดเป็นปีภาคที่ใช้งาน',
        sublabel: 'ตั้งเป็นปีการศึกษาและภาคเรียนปัจจุบัน',
        icon: 'check_circle',
        iconType: 'primary',
        dividerAfter: true,
  ...
  onRowContextMenu(event: MouseEvent, item: YearSemItem): void {
    event.preventDefault();
    event.stopPropagation();
    this.selectedContextMenuRecord.set(item);
    this.contextMenuPos.set({ x: event.clientX, y: event.clientY });
    this.isContextMenuOpen.set(true);
  }

  openMenuFromBtn(event: MouseEvent, item: YearSemItem): void {
    event.preventDefault();
    event.stopPropagation();
    this.selectedContextMenuRecord.set(item);
  ...
  readonly totalItems = computed(() => this.filteredRecords().length);
  ...
  async deleteRecord(item: YearSemItem): Promise<void> {
    const confirmed = await this.confirmDialogService.confirm({
      ...
      detail: 'ข้อมูลตารางสอน, อาจารย์ผู้สอน และวิชาที่เปิดสอนทั้งหมดในปีภาคนี้จะถูกลบออกด้วย (สำรองประวัติไว้ใน HIS)',
      confirmText: 'ลบปีการศึกษานี้',
      cancelText: 'ยกเลิก',
      variant: 'danger',
    });
